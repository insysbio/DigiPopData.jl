...
# incorrect input
@test_throws ArgumentError RealMetricMean(100, 2.0, -0.5) # negative sd
@test_throws ArgumentError RealMetricMean(100, 2.0, 0.0) # zero sd
@test_throws ArgumentError RealMetricMean(100, 2.0, NaN) # NaN sd
@test_throws ArgumentError RealMetricMean(100, 2.0, Inf) # Inf sd
@test_throws ArgumentError RealMetricMean(100, NaN, 0.5) # NaN mean
@test_throws ArgumentError RealMetricMean(100, Inf, 0.5) # Inf mean

# mismatch_expression tests
model = JuMP.Model()
@variable(model, X[1:101], Bin)

m2 = RealMetricMean(100, 10.0, 1.0)
